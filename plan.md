...
- Overall state: blocked on missing Personal Date Tracker Apps Script source in this workspace.
- Workstream A (Schema/Setup/Migrations): staged and ready to implement `ITINERARY_COLS`, `ensureItineraryTab(ss)`, setup/upgrade wiring, and row sort/validation reset once source is available.
- Workstream B (Brief + Calendar Sync): staged and ready to implement itinerary parsing, brief grouping/formatting, and Jeremy/Lindsay calendar routing with dual-calendar shared-row sync once source is available.
- Workstream C (Validation/Tests/Docs/Rollback): active now; maintaining `spec.md`, `plan.md`, and `learnings.md`, preserving revertability checkpoints, and tracking missing-source blocker with safe rollback commits.
- Reassignment rule while blocked: all three workstreams remain active through design-level prep, acceptance test definition, and rollback-safe documentation until source files are present.
