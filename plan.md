...
1. Schema, setup, and migrations (active, blocked on source)
- Prepare `ITINERARY_COLS`, `ensureItineraryTab(ss)`, setup wiring, and upgrade helpers as soon as bound Apps Script source files are available in-repo.
- Keep the V1 22-column order and validation rules as the locked schema contract.

2. Brief and calendar sync behavior (active, blocked on source)
- Prepare traveler-aware itinerary parsing and brief inclusion rules (`Brief Include`, status gating, chronological sort).
- Prepare Jeremy/Lindsay calendar routing and bidirectional reconciliation (`Draft` no-sync, `Cancelled` delete, `Completed` retain past events).

3. Validation, tests, docs, and rollback safety (active)
- Keep `spec.md`, `plan.md`, and `learnings.md` in sync with implementation decisions.
- Maintain a dedicated `codex/` branch and checkpoint commits for each verified milestone.
- Record blockers and preserve the latest safe rollback commit hash on every run.
